...
function positionSuccess({coords}){
  getWeather(coords.latitude, coords.longitude, Intl.DateTimeFormat().resolvedOptions().timeZone).then(renderCurrentWeather).catch(e => {
    console.error(e)
...
function renderCurrentWeather(current_weather){
  ...
  currentTemperature.innerHTML = Math.round(current_weather.main.temp) + "&deg";
  // currentFeelsLike.innerHTML = "Feels like: " + current_weather.main.feels_like + "&deg";
  currentHighTemp.innerHTML = "Highest: " + Math.round(current_weather.main.temp_max) + "&deg";
  currentLowTemp.innerHTML = "Lowest: " + Math.round(current_weather.main.temp_min) + "&deg";
  currentHumidity.innerHTML = "Humidity: " + Math.round(current_weather.main.humidity) + "%";
...
function SetCurrentWeatherIcon(current_weather){
  currentWeatherIcon.src = `https://openweathermap.org/img/wn/${current_weather.weather[0].icon}@2x.png`;
